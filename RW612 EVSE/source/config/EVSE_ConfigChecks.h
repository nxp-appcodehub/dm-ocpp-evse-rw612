...
#if ((EASYEVSE_EV == 0) && (ENABLE_EV_UI == 1))
#error "Cannot enable UI on EV if the EV configuration is not selected"
#endif

#if ((EASYEVSE_EVSE_UI == 1) && (ENABLE_EV_UI == 1))
#error "Choose only one UI version"
#endif

#if ((ENABLE_LCD == 0) && ((ENABLE_EVSE_UI == 1) || (ENABLE_EV_UI == 1)))
#error "Enable the display when enabling a UI"
#endif

#if ((ENABLE_LCD == 1) && (ENABLE_EVSE_UI == 0) && (ENABLE_EV_UI == 0))
