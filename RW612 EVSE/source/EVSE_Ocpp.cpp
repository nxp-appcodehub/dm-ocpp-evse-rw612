...
#define APP_EVSE_OCPP_PRIORITY_OSA PRIORITY_RTOS_TO_OSA(APP_EVSE_OCPP_PRIORITY)
...
static void ocpp_task(osa_task_param_t arg);

static OSA_TASK_DEFINE(ocpp_task, APP_EVSE_OCPP_PRIORITY_OSA, 1, APP_EVSE_OCPP_STACK_SIZE, 0);
static OSA_EVENT_HANDLE_DEFINE(ocpp_event_handle);
static OSA_SEMAPHORE_HANDLE_DEFINE(ocpp_sem_handle);
static OSA_TASK_HANDLE_DEFINE(ocpp_task_handle);
...
static void EVSE_VehicleData_to_OcppData(void)
{
	...
	const phase_data_t urms = Metrology_ReadVoltage();
	const phase_data_t irms = Metrology_ReadCurrent();
	const phase_data_t power = Metrology_ReadActivePower();
	...
	ocppCurrentPowerData.power_W = ocpp::Power{static_cast<float>(power.ph1 + power.ph3 + power.ph2),
		power.ph1, power.ph2, power.ph3};
	ocppCurrentPowerData.voltage_V = ocpp::Voltage{urms.ph1, urms.ph1, urms.ph2, urms.ph3};
	ocppCurrentPowerData.current_A = ocpp::Current{irms.ph1, irms.ph1, irms.ph2, irms.ph3};
    ocppCurrentPowerData.frequency_Hz = ocpp::Frequency{50.0f};
}
...
void EVSE_OCPP_SetEvent(ocpp_events_t event)
{
	...
	OSA_EventSet((osa_event_handle_t)ocpp_event_handle, (osa_event_flags_t)event);
}

void EVSE_OCPP_SetReconnectEvent(void)
{
	EVSE_OCPP_SetEvent(EVSE_RECONNECT_EVENT);
}
...
bool EVSE_OCPP_GetAuthResponse(uint32_t timeout_ms, bool* auth_response)
{
	...
	if (OSA_SemaphoreWait((osa_semaphore_handle_t)ocpp_sem_handle, timeout_ms) == KOSA_StatusSuccess)
	{
        if (auth_response)
        {
...
RequestStartStopStatusEnum OCPP_StopTransaction_Cb(const int32_t evse_id, const ReasonEnum& stop_reason)
{
	...
	if (stop_reason == ReasonEnum::Remote)
	{
		...
		EVSE_OCPP_SetEvent(EVSE_CHARGING_STOP_REMOTE_EVENT);
                                             ...
                                             const bool authorize_remote_start)
{
	...
	else
	{
		...
		EVSE_OCPP_SetEvent(EVSE_CHARGING_START_REMOTE_EVENT);
	}
		...
		const TransactionEventResponse& transaction_event_response)
{
	...
	if (transaction_event_response.idTokenInfo.has_value())
	{
		...
		OSA_SemaphorePost((osa_semaphore_handle_t)ocpp_sem_handle);
...
void ocpp_task(void *arg)
{
	...
	while(true)
	{
		OSA_EventWait(ocpp_event_handle, EVSE_OCPP_ALL_EVENTS, pdFALSE, 1000, &ocppBits);
	...
	is_init = false;
	OSA_EventDestroy((osa_event_handle_t)ocpp_event_handle);
	OSA_TaskDestroy((osa_task_handle_t)ocpp_task_handle);
...
void EVSE_OCPP_Init(void)
{
	...
	status = OSA_EventCreate((osa_event_handle_t)ocpp_event_handle, 1);
	...
	status = OSA_SemaphoreCreateBinary((osa_semaphore_handle_t)ocpp_sem_handle);

    if (status != KOSA_StatusSuccess)
    {
        configPRINTF(("Failed to create binary semaphore !\r\n"));
        while (1);
    }

    status = OSA_TaskCreate((osa_task_handle_t)ocpp_task_handle, OSA_TASK(ocpp_task), NULL);

    if (status != KOSA_StatusSuccess)
    {
        configPRINTF(("Failed to create ocpp Task!\r\n"));
        while (1);
    }
